...
{
	FILE* pfile = 0;
	if (_wfopen_s(&pfile, m_path_name.c_str(), L"rb") != 0)
		return;
...
int LSMReader::LoadOffset(int offset)
{
...
Nrrd* LSMReader::Convert(bool get_max)
{
	return Convert(0, 0, get_max);
}

Nrrd* LSMReader::Convert(int c, bool get_max)
{
		...
		case 1://8-bit
			{
				...
				{
					if (m_l4gb?
						_fseeki64(pfile, ((__int64((*cinfo)[i].offset_high))<<32)+(*cinfo)[i].offset, SEEK_SET)==0:
