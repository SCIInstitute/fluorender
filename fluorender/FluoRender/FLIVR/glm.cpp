...
GLboolean glmLoadTGA(TextureImage *texture, char *filename,GLuint *textureID)			// Loads A TGA File Into Memory
{
	...
	FILE *file;
	if (fopen_s(&file, filename, "rb")!=0)					// Open The TGA File
		return false;
...
static GLvoid glmReadMTL(GLMmodel* model, char* name)
{
	...
	strcpy_s(filename, size_fn, dir);
	strcat_s(filename, size_fn, name);
	free(dir);
	...
	{
		if (!sscanf_s(line, "%s", buf, sizeof(buf)))
			break;
	...
	while(sgets(line, sizeof(line), &mem_file))
	{
		...
		case 'n':               /* newmtl */
			{
				char mtlname[128];
				sscanf_s(line, "%s %s", mtlname, sizeof(mtlname),
					mtlname, sizeof(mtlname));
				nummaterials++;
				model->materials[nummaterials].name = _strdup(mtlname);
			}
			break;
		case 'N':
			if (buf[1] == 's')
			{
				sscanf_s(line, "%s %f", buf, sizeof(buf),
		...
		case 'm':
			{
				char texname[128];
				sscanf_s(line, "%s %s", texname, sizeof(texname), texname, sizeof(texname));
				dir = glmDirName(model->pathname);
				int size_tn = int(strlen(dir) + strlen(texname) + 1);
				filename = (char*)malloc(sizeof(char) * size_tn);
				strcpy_s(filename, size_tn, dir);
				strcat_s(filename, size_tn, texname);
				free(dir);
		...
		case 'K':
			...
			{
			case 'd':
				sscanf_s(line, "%s %f %f %f", buf, sizeof(buf),
...
static GLvoid glmWriteMTL(GLMmodel* model, char* modelpath, char* mtllibname)
{
	...
	strcpy_s(filename, filename_len, dir);
	strcat_s(filename, filename_len, mtllibname);
	free(dir);
...
static GLvoid glmFirstPass(GLMmodel* model, char* file) 
{
	...
	while (sgets(line, sizeof(line), &file))
	{
		...
		case 'm':
			{
				...
				sscanf_s(line, "%s %s", mtlname, sizeof(mtlname),
					mtlname, sizeof(mtlname));
				model->mtllibname = _strdup(mtlname);
		...
		case 'u':
			break;
		case 'g':               /* group */
			sscanf_s(line, "%s %s", buf, sizeof(buf),
				buf, sizeof(buf));
...
GLvoid glmDraw(GLMmodel* model, GLuint mode, bool ext_color, int limit)
{
	...
	{
		line = &(model->lines[i]);
		if (limit<0 || (limit >= 0 && int(line->numvertices) > limit))
		{
			hue = i % 360;
			c = Color(HSVColor(hue, 1.0, 1.0));
			glColor3d(c.r(), c.g(), c.b());
			if (line->numvertices == 1)
			{
				glBegin(GL_POINTS);
				glVertex3fv(&model->vertices[3 * line->vindices[0]]);
				glEnd();
			}
			else
			{
				glBegin(GL_LINE_STRIP);
				for (j=0; j<line->numvertices; j++)
					glVertex3fv(&model->vertices[3 * line->vindices[j]]);
				glEnd();
			}
		}
	}
	...
	while (group)
	{
		...
		if (material)
		{
			...
			if (material->havetexture && (mode&GLM_TEXTURE))
			{
		...
		if (material && material->havetexture)
		{
			glActiveTexture(GL_TEXTURE0);
