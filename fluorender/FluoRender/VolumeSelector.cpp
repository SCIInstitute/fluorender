	...
	VolumeData* vd_g, VolumeData* vd_b, bool select, bool hide)
{
	...
	vd_r->SetColor(Color(1.0, 0.0, 0.0));
	vd_g->SetColor(Color(0.0, 1.0, 0.0));
	vd_b->SetColor(Color(0.0, 0.0, 1.0));
...
void VolumeSelector::GenerateAnnotations(bool use_sel)
{
	...
	for (comp_iter=m_comps.begin(); comp_iter!=m_comps.end(); comp_iter++)
	{
		...
		total_int += intensity;
		wxString str_info = wxString::Format("%d\t%d",
			comp_iter->second.counter, int(intensity+0.5));
	...
	m_annotations->SetMemo(memo.ToStdString());
...
void VolumeSelector::NoiseRemoval(int iter, double thresh, int mode)
{
	...
	else if (mode == 1)
	{
		...
		//2d adjusts
		vd_new->SetGamma(m_vd->GetGamma());
		vd_new->SetBrightness(m_vd->GetBrightness());
		vd_new->SetHdr(m_vd->GetHdr());
