...
void VRenderGLView::DrawVolumes(int peel)
{
		...
		m_force_clear)))
	{
		...
		if (TextureRenderer::get_mem_swap())
		{
			...
			TextureRenderer::set_st_time(GetTickCount());
	...
	if (TextureRenderer::get_mem_swap())
	{
		TextureRenderer::set_consumed_time(GetTickCount() - TextureRenderer::get_st_time());
...
void VRenderGLView::CompExport(int mode, bool select)
{
	...
	if (vr_frame)
	{
		...
		if (group)
		{
			...
			if (vd)
			{
				group->SetGammaAll(vd->GetGamma());
				group->SetBrightnessAll(vd->GetBrightness());
				group->SetHdrAll(vd->GetHdr());
			}
		}
		vr_frame->UpdateList();
			...
			type==9)
		{
			...
			if (vr_frame)
			{
				...
				if(vd_a)
				{
					...
					vd->SetBrightness(vd_a->GetBrightness());
					vd->SetHdr(vd_a->GetHdr());
...
void VRenderGLView::DrawColormap()
{
	...
	else if (num > 1)
	{
		...
		if (vr_frame)
		{
			...
			if (vd && vd->GetDisp())
			{
				VolumeData* vd_view = GetVolumeData(vd->GetName());
...
void VRenderGLView::DrawInfo(int nx, int ny)
{
	...
	{
		sprintf_s(str, 128,
			"FPS: %.2f, Bricks: %d, Quota: %d, Int: %s, Time: %d",
		...
		sprintf_s(str, 128, "FPS: %.2f", fps_>=0.0&&fps_<300.0?fps_:0.0);
	beginRenderText(nx, ny);
	...
	if (m_draw_coord)
	{
		...
		{
			sprintf_s(str, 128, "T: %d  X: %.2f  Y: %.2f  Z: %.2f", m_tseq_cur_num, p.x(), p.y(), p.z());
			beginRenderText(nx, ny);
	...
	{
		sprintf_s(str, 128, "T: %d", m_tseq_cur_num);
		beginRenderText(nx, ny);
		renderText(10, 40, BITMAP_FONT_TYPE_HELVETICA_12, str);
		endRenderText();
	}
	...
	if (m_test_wiref)
	{
		...
		{
			sprintf_s(str, 128, "SLICES: %d", m_mvr->get_slice_num());
			beginRenderText(nx, ny);
		...
		else
		{
			for (int i=0; i<(int)m_vd_pop_list.size(); i++)
			{
				...
				if (vd && vd->GetVR())
				{
					sprintf_s(str, 128, "SLICES_%d: %d", i+1, vd->GetVR()->get_slice_num());
					beginRenderText(nx, ny);
...
double VRenderGLView::GetPointVolumeBox(Point &mp, int mx, int my, VolumeData* vd)
{
	...
	{
		plane = (*planes)[i];
		if (ray.planeIntersectParameter(plane->normal(), plane->get_point(), t))
		{
			pp = ray.parameter(t);
		...
		else if (mode_switch_type == 2)
		{
			if (GetGroupNum() == 1)
			{
				for (int i=0; i<GetLayerNum(); i++)
				{
					...
					{
						DataGroup* group = (DataGroup*)layer;
						group->SetGammaAll(m_glview->GetGamma());
						group->SetBrightnessAll(m_glview->GetBrightness());
						group->SetHdrAll(m_glview->GetHdr());
...
void VRenderView::UpdateView(bool ui_update)
{
	...
	rotx = _wtof(str_val.fn_str());
	str_val = m_y_rot_text->GetValue();
	roty = _wtof(str_val.fn_str());
...
{
	wxString str_val = m_x_rot_text->GetValue();
	int value = _wtoi(str_val.fn_str())+1;
