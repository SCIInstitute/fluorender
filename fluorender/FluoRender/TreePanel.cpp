			...
			case 2://volume data
				{
					if (vr_frame->GetAdjustView())
					{
						...
						if (par_item.IsOk())
						{
							...
							{
								//par is group
								VRenderView* vrv = vr_frame->GetView(GetItemText(GetItemParent(par_item)));
								if (vrv)
								{
									VolumeData* vd = vr_frame->GetDataManager()->GetVolumeData(name);
									DataGroup* group = vrv->GetGroup(GetItemText(par_item));
							...
							{
								//par is view
								VRenderView* vrv = vr_frame->GetView(GetItemText(par_item));
								if (vrv)
								{
									VolumeData* vd = vr_frame->GetDataManager()->GetVolumeData(name);
...
void DataTreeCtrl::OnEndDrag(wxTreeEvent& event)
{
		...
		dst_par_item.IsOk() && vr_frame)
	{
				 ...
				 src_name != dst_name)
		{
			...
			VRenderView* vrv = vr_frame->GetView(GetItemText(GetItemParent(src_par_item)));
			if (vrv)
...
void DataTreeCtrl::ChangeIconColor(int which, wxColor c, int type)
{
	...
	char **orgn_data = type?tick_xpm:cross_xpm;
	char cc[8];

	int dummy;
	sscanf_s(orgn_data[0], "%d %d %d %d",
