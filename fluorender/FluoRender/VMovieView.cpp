...
m_prev_frame(0)
{
	...
	m_views_cmb = new wxComboBox(this, ID_ViewsCombo, "",
		wxDefaultPosition, wxSize(120, 20), 0, NULL, wxCB_READONLY);
	...
	m_x_rd = new wxRadioButton(this, ID_XRd, "X",
		wxDefaultPosition, wxSize(80, 20), wxRB_GROUP);
	m_y_rd = new wxRadioButton(this, ID_YRd, "Y",
		wxDefaultPosition, wxSize(60, 20)/*, wxRB_GROUP*/);
	...
	st = new wxStaticText(this, 0, "minus (-)",
		wxDefaultPosition, wxSize(50, 15));
	sizer_4->Add(80, 5, 0);
...
void VMovieView::OnStartAngleEditing(wxCommandEvent& event)
{
	if (m_init)
	{
		...
		double start_angle = -fabs(_wtof(str.fn_str()));
		str = m_angle_end_text->GetValue();
		double end_angle = _wtof(str.fn_str());
		str = m_step_text->GetValue();
		double step = _wtof(str.fn_str());
...
void VMovieView::OnPrev(wxCommandEvent& event)
{
	...
	start_angle = -fabs(_wtof(str.fn_str()));
	str = m_angle_end_text->GetValue();
	end_angle = _wtof(str.fn_str());
	str = m_step_text->GetValue();
	step = _wtof(str.fn_str());
	if (step < 0.01)
		step = 0.01;
	if (m_x_rd->GetValue())
		rot_axis = 1;
...
void VMovieView::OnFrameSpinUp(wxSpinEvent& event)
{
	...
	if (text_ctrl)
	{
		...
		char str[256];
		sprintf(str, "%d", _wtoi(str_val.fn_str())+1);
