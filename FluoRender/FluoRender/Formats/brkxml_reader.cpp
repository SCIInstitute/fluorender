...
BRKXMLReader::BRKXMLReader()
{
   ...
   m_ex_metadata_path = std::wstring();
   m_ex_metadata_url = std::wstring();
...
void BRKXMLReader::LoadROITree(tinyxml2::XMLElement *lvNode)
{
        ...
        int gid = -2;
        LoadROITree_r(lvNode, m_roi_tree, std::wstring(L""), gid);
}
...
void BRKXMLReader::LoadROITree_r(tinyxml2::XMLElement *lvNode, std::wstring& tree, const std::wstring& parent, int& gid)
{
        ...
        while (child)
        {
                if (child->Name() && child->Attribute("name"))
                {
                        ...
                        {
			  std::cerr << "BRKXMLReader::LoadROITree_r(XMLElement *lvNode, std::wstring& tree, const std::wstring& parent): bad_lexical_cast" << std::endl;
                        }
                }
                child = child->NextSiblingElement();
        }
