...
public:
	...
	void build_pyramid(std::vector<flvr::Pyramid_Level> &pyramid, std::vector<std::vector<std::vector<std::vector<flvr::FileLocInfo *>>>> &filenames, int t, int c);
	void OutputInfo();
	...
	bool loadMetadata(const std::wstring &file = std::wstring());
...
private:
	...
	void ReadFilenames(tinyxml2::XMLElement* fileRootNode, std::vector<std::vector<std::vector<flvr::FileLocInfo *>>> &filename);
